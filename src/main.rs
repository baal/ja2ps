...
fn ja2ps(in_file_path: Option<String>, out_file_path: Option<String>) -> Result<(), io::Error> {
    ...
    let mut w: io::BufWriter<Box<dyn Write>> = io::BufWriter::new(match out_file_path {
    ...
    for line in r.lines() {
        ...
        for buf in &bufs {
            if newpage {
                ...
                w.write(
                    format!(
                        "/GothicBBB-Medium-Identity-H findfont {font_size} scalefont setfont\n",
                        font_size = font.size
                    )
                    .as_bytes(),
                )?;
